...
require_text_in_file "$factory_rust_builder_workflow" "cargo check --manifest-path examples/downstream-smoke/Cargo.toml" "factory rust-builder workflow should check downstream-smoke when its paths select validation"
reject_text_in_file "$factory_rust_builder_workflow" '      - "fuzz/Cargo.toml"' "factory rust-builder workflow should not select the fuzz workspace without a C++ toolchain"
reject_text_in_file "$factory_rust_builder_workflow" '      - "fuzz/Cargo.lock"' "factory rust-builder workflow should not select fuzz lockfile changes without a C++ toolchain"
reject_text_in_file "$factory_rust_builder_workflow" "cargo check --manifest-path fuzz/Cargo.toml" "factory rust-builder workflow should not build the fuzz workspace without a C++ toolchain"
